...
const UserTasks = () => {
    ...
    useEffect(() => {
        fetch("https://sheltered-citadel-68723.herokuapp.com/showUserTasks?email=" + loggedInUser.email, {
            method: "GET",
            headers: {
                ...
                <div className="row mt-4">
                    {userTasks.map((userTask) => (
                        <div className="col-sm-6 col-md-4 col-lg-3" key={Math.random()}>
                            ...
                            <button className="btn btn-warning" onClick={() => cancelTask(`${userTask._id}`)}>
                                Cancel
                            </button>
                        </div>
                    ))}
